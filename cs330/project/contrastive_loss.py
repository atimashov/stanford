...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # find l(k, k+N)
        sum_exp_k = sum([torch.exp(sim(z_k, z_x) / tau) for z_x in out.unbind()]) - torch.exp(torch.ones(1) / tau)
        l_k_kN = -sim(z_k, z_k_N) / tau + torch.log(sum_exp_k)

        # find l(k+N, k)
        sum_exp_kN = sum([torch.exp(sim(z_k_N, z_x) / tau) for z_x in out.unbind()]) - torch.exp(torch.ones(1) / tau)
        l_kN_k = -sim(z_k_N, z_k) / tau + torch.log(sum_exp_kN)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(dim = 1, keepdim = True) / (norm_left * norm_right)
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out_norm, out_norm.T) # 2N x 2N
    return sim_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Compute the denominator values for all augmented samples. This should be a 2N x 1 vector.
    ...
    numerator = torch.cat([sim_pos / tau, sim_pos / tau], dim=0)

    # Step 4: Compute the total loss.
    loss = None
    loss = (-numerator + torch.log(denom)).sum() / (2 * N)
    return loss
