...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 2. Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`
        X = pack_padded_sequence(X, source_lengths) # TODO: clarify what this function does
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first = True) # TODO: clarify what this function does
        # 3. Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        enc_last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim = 1)
        init_decoder_hidden = self.h_projection(enc_last_hidden)
        enc_last_cell = torch.cat((last_cell[0], last_cell[1]), dim = 1)
        init_decoder_cell = self.c_projection(enc_last_cell)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3. Compute the attention scores e_t
        dec_hidden_t = torch.unsqueeze(dec_hidden, 2) # pretend that transpose
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, dec_hidden_t), 2) # remove '1' to get b x src_len
        ### END YOUR CODE
        ...
        a_t = torch.sum(torch.unsqueeze(alpha_t, 2) * enc_hiddens, dim = 1)
        # 3. Concatenate dec_hidden with a_t to compute tensor U_t
